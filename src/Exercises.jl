...
# 
#    set_score("exercise identifier",score)

using Random

# 3. Interaction
# 3.5.1 Install all packages for the course
...
check_functions["7.4.1"] = function(result)
    ...
    @assert out != ["foofoofoo","zackzackzack"] "Space missing"
...
check_functions["7.4.5"] = function(result)
    ...
    @assert length(out) > 0 "Output missing"
    for (idx,i) in enumerate(1:3:length(out)-3)
        r = inp[idx]
        @assert out[i] == "r = $(round(r,digits=2))" "Error in output."
        @assert out[i+1] == "A = $(round(r^2*π,digits=2))" "Error in output or calculation of A."
        @assert out[i+2] == "U = $(round(2*r*π,digits=2))" "Error in output or calculation of U."
    end
end

# 8. Arrays, Dicts, Tuples
# 8.4.1 Concatenate three arrays
...
check_functions["8.5.4"] = function(result)
    ...
    for _ in 1:100
        ...
        @assert result(w,join(shuffle(collect(w)))) "Incorrect output. Word is an anagram."
        @assert !result(w,wd) "Incorrect output. Word is not an anagram."
    end
    @assert result("abc","CaB") "Incorrect output. Case should be ignored."
end

# 8.4.5 Print the N-th element of a dictionary
...
using LinearAlgebra
# 10.3.1 Unique solvability of linear systems
check_functions["10.3.1"] = function(solvable)
    @assert solvable([1 -1
            2  3],[22, 11])
    @assert !solvable([1 -1
            2  3
    ...
    @assert !solvable([1 1
                    2 2],[2, 3])
    @assert solvable(rand(3,3),rand(3))
end

# 10.3.2 Solve the system of equations
...
# 10.3.3 Eigenvalues and eigenvectors

check_functions["10.3.3"] = function(reconstruct)
    As = [rand(x,x) for x in [rand(3:10) for _ in 1:10]]
    λ = eigvals.(As)
    ϕ = eigvecs.(As)
    @assert all(reconstruct.(λ,ϕ) .≈ As) "Error reconstructing the matrix."
end

# 10.3.3 Write a function to calculate the angle between two vectors v1 and v2

check_functions["10.3.4"] = function(result)
    function check(v1,v2)
        res = result(v1,v2)
        @assert isa(res,Number) "The function should return an angle."
        eq = res ≈ acos(v1⋅v2/(norm(v1)*norm(v2)))
        @assert eq  "Incorrect angle calculation for vectors $v1 and $v2. Note: The angle must be calculated in radians."
    end
    check([1,2,3],[4,5,6])
    check([3.,1.,9.],[4.,-1.0,-3.])
    check([1,0,0],[1,0,0])
...
check_functions["11.3.1"] = function(result)
    function testit(number,expected)
        ...
        @assert length(out) == 1 "More than one line of output!"
        @assert first(out) == expected "Expected output for '$number' is '$expected'."
    end
    numbers = [rand(2:2:100) for _ in 1:10]
    testit.(numbers,Ref("Even"))
    numbers = [rand(1:2:101) for _ in 1:10]
    testit.(numbers,Ref("Odd"))
end
...
check_functions["11.3.2"] = function(result)
    ...
    expected = ["Enter radius: A = 28.27", "U = 18.85", "", "Enter radius: A = 3.8", "U = 6.91", "", "Enter radius: Invalid input", "", "Enter radius: A = 254.47", "U = 56.55", "", "Enter radius: "]
    for (i,l) in enumerate(expected)
        @assert i <= length(out) "Expected number of lines does not match."
        @assert out[i] == l "Expected: '$(expected[i])' received: '$(out[i])'"
    end
end

check_functions["11.3.3"] = function(result)
    @assert count([floor(result(1000),digits=0) for _ in 1:100] .== 3.0) > 90 "Incorrect calculation"
    @assert count([floor(result(1_000_000),digits=1) for _ in 1:20] .== 3.1) > 15 "Incorrect calculation"
end

check_functions["11.3.4"] = function(result)
    function check(f,a,b,expected,fname)
        x = collect(a:0.0001:b)
        @assert isapprox(result(x,f.(x)),expected,atol=1e-4) "Incorrect result for the function '$fname' from '$a' to '$b'"
    end
...
check_functions["11.3.5.1"] = function(result)
    ...
    @assert all(inrange.([floor(result(10000000),digits=4) for _ in 1:10])) "Error in calculation"
    calcs = [floor(result(10^n),digits=4) for n in 0:3]
    @assert std(calcs) != 0.0 "No change with increasing iterations."
end
...
check_functions["11.3.6"] = function(result)
    ...
    for i in 1:10
        ...
        @assert result(t.sentence,t.sep) == t.split "Error with call using '$(t.sentence)' and '$(t.sep)'."
    end
    t = generate_random_sentence(1)
    @assert result(t.sentence,t.sep) == t.split "Error with call using '$(t.sentence)' and '$(t.sep)'."
    @assert result(t.sep*t.sentence*t.sep,t.sep) == t.split "Error with call using '$(t.sep*t.sentence*t.sep)' and '$(t.sep)'."
    @assert result(t.sep*t.sentence*t.sep*t.sep*t.sentence,t.sep) == [t.sentence, t.sentence] "Error with call using '$(t.sep*t.sentence*t.sep*t.sep*t.sentence)' and '$(t.sep)'."
    @assert result(t.sentence,'+') == t.split "Error with call using '$(t.sentence)' and '+'."
end
dont!("11.3.6", :split)
...
check_functions["11.3.7"] = function(bsqrt)
    for _ in 1:100
        ...
        @assert isapprox(bsqrt(n),sqrt(n),atol=1e-5) "Error in calculation for '$n'."
    end
end
dont!("11.3.7", :sqrt)

# 11.3.8 Merge dictionaries
...
# 12. Debugger
# 12.3.1 gcd
check_functions["12.3.1"] = function(gcd)
    a = rand(1:1000,1000); b = rand(1:1000,1000);
    @assert gcd.(a,b) == gcd.(a,b) "Error in calculation"
    @assert gcd(10,0) == gcd(10,0)
    @assert gcd(0,10) == gcd(0,10)
    @assert gcd(-2304,288) == gcd(-2304,288)
end
dont!("12.3.1", :gcd)
...
do!("13.3.1", :Date, :Day, :Week)

# 14. FileIO
# 14.3.1 Survival camp
const stuff = ["Pocket Knife",
             "Flashlight",
             "Battery",
             "Water Bottle",
             "Sleeping Bag",
...
function generate_survival_camp()
    ...
    for (i,lager) in enumerate(lagers)
        save_lager(lager,joinpath(path,"Camp$i"))
    end
    return lagers
end
...
check_functions["14.3.1"] = function(read_lagers)
    ...
    for _ in 1:10
        ...
        inventory = read_lagers()
        for (i, lager) in enumerate(camp)
            for thing in all_things_in_lager(lager)
                tot = total_of_thing(lager, thing)
                @assert thing in keys(inventory[i]) "Expecting $tot units of $thing in Lager $i, no entry found.\nExample files under '$(pwd())'"
                @assert tot == inventory[i][thing] "Incorrect quantity in Lager $i for $thing. Expecting $tot, received $(inventory[i][thing]).\nExample files under: '$(pwd())'"
            end
        end
    end
end
...
check_functions["14.3.2"] = function(lager_mit)
    Random.seed!(RandomDevice())
    function get_lager_ids_with(camp, item, amount)
        ids = []
        for (id, lager) in enumerate(camp)
            for thing in all_things_in_lager(lager)
                tot = total_of_thing(lager, thing)
                if thing == item
                    tot >= amount && push!(ids, id)
                    break
                end
            end
        end
    ...
    for _ in 1:10
        ...
        for i in 1:100
            ...
            @assert idsA == idsB "Incorrect IDs. Expected $idsA, got $idsB."
        end
    end
end
do!("14.3.2", :joinpath)

check_functions["14.3.3"] = function(total_inventory)
    Random.seed!(RandomDevice())
    generate_survival_camp()
    for _ in 1:10
        camp = generate_survival_camp()
        ...
        invB = total_inventory()
        @assert invB == total "Incorrect inventory. Expected $total, got $invB."
    end
end
do!("14.3.3", :joinpath)
...
"10.3.1" => [120, 118, 108, 113, 106, 35, 79, 108, 113, 104, 100, 117, 68, 111, 106, 104, 101, 117, 100, 13, 13, 35, 35, 35, 35, 105, 120, 113, 102, 119, 108, 114, 113, 35, 118, 114, 111, 121, 100, 101, 111, 104, 43, 68, 47, 101, 44, 13, 35, 35, 35, 35, 35, 35, 35, 35, 117, 100, 113, 110, 43, 68, 44, 35, 64, 64, 35, 117, 100, 113, 110, 43, 107, 102, 100, 119, 43, 68, 47, 101, 44, 44, 13, 35, 35, 35, 35, 104, 113, 103],
...
"7.4.2" => [105, 120, 113, 102, 119, 108, 114, 113, 35, 102, 107, 100, 108, 113, 43, 105, 47, 35, 106, 47, 35, 123, 44, 13, 35, 35, 35, 35, 117, 104, 119, 120, 117, 113, 35, 105, 43, 106, 43, 123, 44, 44, 13, 104, 113, 103],
...
"10.3.4" => [120, 118, 108, 113, 106, 35, 79, 108, 113, 104, 100, 117, 68, 111, 106, 104, 101, 117, 100, 13, 105, 120, 113, 102, 119, 108, 114, 113, 35, 100, 113, 106, 111, 104, 43, 121, 52, 47, 121, 53, 44, 13, 35, 35, 35, 35, 100, 102, 114, 118, 43, 121, 52, 8904, 121, 53, 50, 43, 113, 114, 117, 112, 43, 121, 52, 44, 45, 113, 114, 117, 112, 43, 121, 53, 44, 44, 44, 13, 104, 113, 103],
...
"14.3.1" => [120, 118, 108, 113, 106, 35, 70, 86, 89, 13, 105, 120, 113, 102, 119, 108, 114, 113, 35, 117, 104, 100, 103, 98, 102, 100, 112, 115, 43, 44, 13, 35, 35, 35, 35, 115, 100, 119, 107, 35, 64, 35, 37, 52, 55, 48, 54, 37, 13, 35, 35, 35, 35, 105, 108, 111, 104, 118, 35, 64, 35, 117, 104, 100, 103, 103, 108, 117, 43, 115, 100, 119, 107, 44, 13, 35, 35, 35, 35, 114, 120, 119, 115, 120, 119, 35, 64, 35, 71, 108, 102, 119, 126, 76, 113, 119, 47, 71, 108, 102, 119, 126, 86, 119, 117, 108, 113, 106, 47, 35, 76, 113, 119, 128, 128, 43, 44, 13, 35, 35, 35, 35, 105, 114, 117, 35, 105, 35, 108, 113, 35, 105, 108, 111, 104, 118, 13, 35, 35, 35, 35, 35, 35, 35, 35, 116, 120, 100, 113, 119, 108, 119, 124, 35, 64, 35, 71, 108, 102, 119, 126, 86, 119, 117, 108, 113, 106, 47, 76, 113, 119, 128, 43, 44, 13, 35, 35, 35, 35, 35, 35, 35, 35,